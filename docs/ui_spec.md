...
| **ミニマム統計**: グループ総数、消費しているタブ数 | **[一括掃除]**: CleanupPageへ直接遷移 |
...
| **表示項目 (Information)** | **操作項目 (Action)** |
| --- | --- |
| **マージ元候補エリア**: 同名・類似URLの保存済みグループ群 | **チェックボックス**: 統合したい「古い残骸」を選択 |
| **マージ先指定エリア**: 現在開いている（Active）グループ群 | **ラジオボタン**: どのグループに情報を集約するか1つ選択 |
| **マージ後予測**: 合計されるタブ数、重複排除されるURL数 | **[マージを実行]**: 指定したActiveグループへ統合し、他を消去 |
...
| **削除対象プレビュー**: 条件に合致したグループをグリッド表示 | **個別除外ボタン**: 自動判定されたが「残したい」ものを外す |
| **保護対象の明示**: 他デバイス（モバイル）データの保護状態 | **[XX件を今すぐ削除]**: 確定したリストを一括抹消 |
...
| **表示項目 (Information)** | **操作項目 (Action)** |
| --- | --- |
| **整理実績**: 「XX個の重複を解消」「XX個の空グループを削除」 | **[ダッシュボードへ戻る]**: 他の整理機能へ |
