...
def verify_no_external_libraries():
    ...
    # Allowed external domains (for fonts, etc. as per Solo policy clarification)
        ...
        for file in files:
            if file.endswith(".html"):
                ...
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    for match in external_script_src_pattern.finditer(content):
                        url = match.group(1)
                        if not any(domain in url for domain in allowed_domains):
                            found_external = True
                            print(f"Error: External script found in {file_path}: {url}")

                    for match in external_link_href_pattern.finditer(content):
                        url = match.group(1)
                        if not any(domain in url for domain in allowed_domains):
                            found_external = True
                            print(f"Error: External link found in {file_path}: {url}")
